...
export const con = mysql.createConnection({ host: config.host, port: 3306, user: config.user, password: config.password, database: config.database }).promise();
...
export const register = async (username: string, password: string) => {
    try {
        ...
        await con.query(`INSERT INTO User(username, password, private_key) VALUES('${username}','${passwordEncrypted}','${private_key}')`);
