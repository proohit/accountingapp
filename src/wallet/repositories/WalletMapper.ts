...
export const byName = async (name: string, owner: string): Promise<Wallet> => {
  try {
    const wallet = await con.query<Wallet[]>(
...
export const byUser = async (owner: string): Promise<Wallet[]> => {
  try {
    const wallets = await con.query<Wallet[]>(
      `SELECT * FROM Wallet WHERE owner='${owner}'`
    );
    return wallets[0];
  } catch (error) {
    throw error;
...
export const create = async (
  ...
  try {
    await con.query<Wallet[]>(
      `INSERT INTO Wallet(name, balance, owner) VALUES('${name}',${balance},'${owner}') `
    );
