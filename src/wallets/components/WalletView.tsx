import { Grid, List, Typography, Paper } from '@material-ui/core';
...
import { deleteWallet, editWallet, getAllWallets, postWallet } from '../service/WalletService';
...
class WalletView extends React.Component<WithStyles<typeof WalletViewStyles> & IWalletViewProps, IWalletViewState> {
    ...
    postWallet = async (item: Wallet) => {
        try {
            ...
            this.fetchWallets();
            this.props.functionSet.openAlert({ message: "Wallet created", severity: Severity.success })
            this.setState({ addDialog: false })
        } catch (error) {
            this.props.functionSet.openAlert({ message: "Error creating Wallet", severity: Severity.error })
            this.setState({ addDialog: false })
    ...
    deleteWallet = async (item: Wallet) => {
        try {
            ...
            this.props.functionSet.openAlert({ message: "Wallet and all connected records deleted", severity: Severity.success })
            this.fetchWallets();
        } catch (error) {
            this.props.functionSet.openAlert({ message: "Error creating Wallet", severity: Severity.error })
        } finally {
            this.setState({ deleteDialog: { open: false, selected: this.state.deleteDialog.selected } })
        }
    };

    editWallet = async (item: Wallet) => {
        try {
            await editWallet(this.context.token, this.state.editDialog.selected, item);
            this.props.functionSet.openAlert({ message: "Updated Wallet", severity: Severity.success })
            this.fetchWallets();
        } catch (error) {
            this.props.functionSet.openAlert({ message: error.message, severity: Severity.error })
        } finally {
            this.setState({ editDialog: { open: false, selected: this.state.editDialog.selected } })
        }
    }
    ...
    fetchWallets = async () => {
        try {
            ...
            this.props.functionSet.openAlert({ message: "Error fetching wallets", severity: Severity.error })
        }
    }
    render() {
            ...
            <div>
                ...
                <Grid container direction="column" justify="flex-start" alignItems="center">
                    ...
                    <DeleteWalletDialog
                        ...
                        onCancel={() => this.setState({ deleteDialog: { open: false, selected: this.state.deleteDialog.selected } })}
                        item={this.state.deleteDialog.selected}
                    />
                    <EditWalletDialog
                        open={this.state.editDialog.open}
                        confirmClickhandler={this.editWallet}
                        onCancel={() => this.setState({ editDialog: { open: false, selected: this.state.editDialog.selected } })}
                    ...
                    <List className={classes.list}>
                        {this.state.wallets.map(wallet =>
                            <WalletCard
                                key={wallet.name}
                                deleteHandler={(wallet) => this.setState({ deleteDialog: { open: true, selected: wallet } })}
                                editHandler={(wallet) => this.setState({ editDialog: { open: true, selected: wallet } })}
                                wallet={wallet} />)}
                    </List>
